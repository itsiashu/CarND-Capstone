    ...
    def process_traffic_lights(self):
        ...
        if(self.pose):
            ...
            for i, light in enumerate(self.lights):
                ...
                cur_diff = temp_waypoint_index - car_position
                if 0 <= cur_diff < diff:
                    diff = cur_diff
                    closest_light = light
                    waypoint_index = temp_waypoint_index
        #TODO find the closest visible traffic light (if one exists)
        if closest_light:
            self.process_count += 1
            state = self.get_light_state(closest_light)
            if (self.process_count % THROTTLE_NO) == 0:
                rospy.logwarn("waypint_index={}, state={}".format(waypoint_index, self.state_str(state)))
